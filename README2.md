...
7.  When ready, follow the *Bundling and publishing a theme steps* (ie: run `stencil bundle`).  IMPORTANT: YOU WANT TO DROP THE LAST COMMIT FOR BUNDLING THE THEME FOR NTL.COM (since it removes the link to redirect to ntl.ca)
8. YOU MAY NEED TO UPDATE THE BANNER (take the existing one from the previous theme) in
`templates/components/common/header.html`
9. To test a specific style locally, type
...
TO ENSURE THE NEW VERSION OF STENCIL IS RUNNING LOCALLY, WRITE `nvm use 18.16.0` BEFORE RUNNING `stencil bundle`.  You will need to do the following for the CleanWebpackBundle error as well:
